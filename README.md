...
衡析（原 AskData Studio）是基于 AskData 原型持续迭代的本地演示项目，面向企业经营数据查询与分析场景。业务人员可以用自然语言查询示例数据库，查看表格、SQL、结果说明，并基于已有结果生成图表或分析报告。

> 当前版本使用仓库中的合成演示数据和固定演示账号，尚未部署为生产系统。不要用它连接敏感或生产数据库。
